...
class RegistrationTaskCreate(BaseModel):
    ...
    codex2api_service_ids: List[int] = []  # 指定 Codex2API 服务 ID 列表
...
def _get_email_service_backoff_state(
...
def _is_email_service_circuit_open(
    service_id: Optional[int], now: Optional[float] = None
) -> bool:
    if service_id is None:
        return False
...
def _run_registration_engine_attempt(
    ...
    with _email_service_backoff_lock:
        if db_service is not None:
            provider_backoff_before_run = _get_email_service_backoff_state(
                db_service.id
            )
            if hasattr(email_service, "apply_provider_backoff_state"):
        ...
        create_cancel_callback = getattr(
            task_manager, "create_check_cancelled_callback", None
        )
...
def _build_email_service_candidates(
    ...
    # 强制从数据库重新加载设置，确保获取最新配置
    settings = reload_settings()
    candidates: List[Dict[str, object]] = []

    def append_candidate(
        candidate_type: EmailServiceType, config: dict, db_service=None
    ) -> None:
        candidates.append(
            {
    ...
    def append_database_candidates(db_service_type: str) -> None:
        ...
        for db_service in services:
            ...
            config = _normalize_email_service_config(
                candidate_type, db_service.config, actual_proxy_url
            )
    ...
    if service_type == EmailServiceType.TEMPMAIL:
        tempmail_config = {
            "base_url": getattr(
                settings, "tempmail_base_url", "https://api.tempmail.lol/v2"
            ),
            "timeout": getattr(settings, "tempmail_timeout", 30),
            "max_retries": getattr(settings, "tempmail_max_retries", 3),
            "proxy_url": actual_proxy_url,
        }

        # 添加 Plus/Ultra 用户的高级配置
        api_key = getattr(settings, "tempmail_api_key", None)
        if api_key:
            tempmail_config["api_key"] = api_key

        custom_domain = getattr(settings, "tempmail_custom_domain", None)
        if custom_domain:
            tempmail_config["custom_domain"] = custom_domain

        enable_subdomain = getattr(settings, "tempmail_enable_random_subdomain", None)
        if enable_subdomain is not None:
            tempmail_config["enable_random_subdomain"] = enable_subdomain

        subdomain_length = getattr(settings, "tempmail_subdomain_length", None)
        if subdomain_length:
            tempmail_config["subdomain_length"] = subdomain_length

        # 调试日志：显示 Tempmail 配置
        logger.info(
            f"[Tempmail] 注册配置: custom_domain={tempmail_config.get('custom_domain')}, "
            f"enable_random_subdomain={tempmail_config.get('enable_random_subdomain')}, "
            f"has_api_key={bool(tempmail_config.get('api_key'))}"
        )

        append_candidate(service_type, tempmail_config)
            ...
            if settings.custom_domain_base_url and settings.custom_domain_api_key:
                append_candidate(
                    service_type,
                    {
                        ...
                        if settings.custom_domain_api_key
                        else "",
                        "proxy_url": actual_proxy_url,
                    },
                )
        ...
        for db_service in services:
            ...
            if existing:
                ...
                continue
            config = _normalize_email_service_config(
                service_type, db_service.config, actual_proxy_url
            )
        ...
        if not candidates:
            raise ValueError(
                "没有可用的 Cloud Mail 邮箱服务，请先在邮箱服务页面添加服务"
            )
    else:
    ...
    with get_db() as db:
        try:
            ...
            task = crud.update_registration_task(
                db, task_uuid, status="running", started_at=datetime.utcnow()
            )
            if not task:
            ...
            log_callback = task_manager.create_log_callback(
                task_uuid, prefix=log_prefix, batch_id=batch_id
            )
            ...
            while True:
                if cancellation_requested():
                    _finalize_task_cancelled(
                        db, task_uuid, email_service=active_service_type.value
                    )
                    return
                ...
                if not actual_proxy_url:
                    ...
                    if actual_proxy_url:
                        logger.info(
                            f"任务 {task_uuid} 使用代理: {actual_proxy_url[:50]}..."
                        )
                ...
                for attempt_index, candidate in enumerate(service_candidates, start=1):
                    ...
                    if db_service is not None:
                        crud.update_registration_task(
                            db, task_uuid, email_service_id=db_service.id
                        )
                        logger.info(
                        ...
                        crud.update_registration_task(
                            db, task_uuid, email_service_id=None
                        )

                    task_manager.update_status(
                        task_uuid, "running", email_service=active_service_type.value
                    )
                    status_callback = _create_task_status_callback(
                        task_uuid, active_service_type.value
                    )
                    ...
                    if callable(create_cancel_callback):
                        set_cancel_callback = getattr(
                            email_service, "set_check_cancelled", None
                        )
                    ...
                    if (
                        ...
                        or result.error_code == ERROR_TASK_CANCELLED
                    ):
                        _finalize_task_cancelled(
                            db,
                            task_uuid,
                    ...
                    can_failover = (
                        ...
                        and email_prepare_phase.error_code
                ...
                if should_retry_with_new_proxy:
                    ...
                    if proxy_id and disable_proxy_for_network_error(
                        db, proxy_id, result.error_message
                    ):
                        exhausted_proxy_ids.add(proxy_id)
                        log_callback(
                            f"[代理] 当前代理已标记失效并从代理池移除: {proxy_id}"
                        )
                    ...
                    if next_proxy_url and (
                        next_proxy_url != actual_proxy_url or next_proxy_id != proxy_id
                    ):
                        requested_proxy = None
                        log_callback(
                            f"[代理] 切换到新代理后重试注册: {next_proxy_url[:50]}..."
                        )
                        continue
            ...
            if result.success:
                ...
                if auto_upload_cpa:
                    try:
                        ...
                        saved_account = (
                            db.query(AccountModel).filter_by(email=result.email).first()
                        )
                        if saved_account and saved_account.access_token:
                            ...
                            if not _cpa_ids:
                                ...
                                _cpa_ids = [
                                    s.id
                                    for s in crud.get_cpa_services(db, enabled=True)
                                ]
                            if not _cpa_ids:
                    ...
                    try:
                        ...
                        if saved_account and saved_account.access_token:
                            ...
                            if not _s2a_ids:
                                _s2a_ids = [
                                    s.id
                                    for s in crud.get_sub2api_services(db, enabled=True)
                                ]
                            if not _s2a_ids:
                            ...
                            for _sid in _s2a_ids:
                                try:
                                    ...
                                    log_callback(
                                        f"[Sub2API] {'成功' if _ok else '失败'}({_svc.name}): {_msg}"
                                    )
                                except Exception as _e:
                ...
                if auto_upload_tm:
                    try:
                        ...
                        if saved_account and saved_account.access_token:
                            ...
                            if not _tm_ids:
                                _tm_ids = [
                                    s.id for s in crud.get_tm_services(db, enabled=True)
                                ]
                            if not _tm_ids:
                            ...
                            for _sid in _tm_ids:
                                try:
                                    ...
                                    _ok, _msg = upload_to_team_manager(
                                        saved_account, _svc.api_url, _svc.api_key
                                    )
                                    log_callback(
                                        f"[TM] {'成功' if _ok else '失败'}({_svc.name}): {_msg}"
                                    )
                                except Exception as _e:
                    ...
                    try:
                        ...
                        if saved_account and saved_account.access_token:
                            ...
                            if not _na_ids:
                                _na_ids = [
                                    s.id
                                    for s in crud.get_newapi_services(db, enabled=True)
                                ]
                            if not _na_ids:
                            ...
                            for _sid in _na_ids:
                                try:
                                    ...
                                    if _ok:
                                        ...
                                        saved_account.newapi_uploaded_at = (
                                            datetime.utcnow()
                                        )
                                        db.commit()
                ...
                if auto_upload_codex2api:
                    try:
                        ...
                        if saved_account and saved_account.refresh_token:
                            _c2a_ids = codex2api_service_ids or []
                            if not _c2a_ids:
                                _c2a_ids = [
                                    s.id
                                    for s in crud.get_codex2api_services(
                                        db, enabled=True
                                    )
                                ]
                            if not _c2a_ids:
                                log_callback(
                                    "[Codex2API] 无可用 Codex2API 服务，跳过上传"
                                )
                            for _sid in _c2a_ids:
                                try:
                                    _svc = crud.get_codex2api_service_by_id(db, _sid)
                                    if not _svc:
                                        continue
                                    log_callback(f"[Codex2API] 上传到服务: {_svc.name}")
                                    _ok, _msg = upload_to_codex2api(
                                        saved_account,
                                        _svc.api_url,
                                        _svc.api_key,
                                        _svc.include_proxy_url,
                                    )
                                    log_callback(
                                        f"[Codex2API] {'成功' if _ok else '失败'}({_svc.name}): {_msg}"
                                    )
                                except Exception as _e:
                                    log_callback(f"[Codex2API] 异常({_sid}): {_e}")
                    except Exception as c2a_err:
                        log_callback(f"[Codex2API] 上传异常: {c2a_err}")
                ...
                logger.warning(
                    f"注册任务失败: {task_uuid}, 原因: {result.error_message}"
                )
...
class _MockBackoffEmailService(BaseEmailService):
    ...
    def __init__(self):
        super().__init__(
            service_type=EmailServiceType.DUCK_MAIL, name="mock-backoff-service"
        )
...
def _create_persisted_log_callback(
    task_uuid: str, prefix: str = "", batch_id: str = ""
):
...
def _simulate_batch_counter_probe(batch_id: str) -> Dict[str, Any]:
    ...
    with get_db() as db:
        for index, (task_uuid, status) in enumerate(
            zip(task_uuids, task_statuses), start=1
        ):
            crud.create_registration_task(db, task_uuid=task_uuid, proxy=None)
            error_message = (
                None if status == "completed" else f"mock-batch-error-{index}"
            )
            crud.update_registration_task(
                db,
                task_uuid,
            ...
            update_batch_status(
                completed=new_completed, success=new_success, failed=new_failed
            )
...
async def run_mock_registration_task(
    ...
    try:
        ...
        task_manager.update_status(
            task_uuid, "running", email_service=email_service_type
        )
        ...
        task_manager.update_status(
            task_uuid, "failed", error=str(exc), email_service=email_service_type
        )
...
async def run_batch_parallel(
    ...
    add_batch_log(
        f"[系统] 并行模式启动，并发数: {concurrency}，总任务: {len(task_uuids)}"
    )
    ...
    async def _run_one(idx: int, uuid: str):
        ...
        async with semaphore:
            await run_registration_task(
                ...
                newapi_service_ids=newapi_service_ids or [],
                auto_upload_codex2api=auto_upload_codex2api,
                codex2api_service_ids=codex2api_service_ids or [],
            )
        with get_db() as db:
    ...
    try:
        await asyncio.gather(
            *[_run_one(i, u) for i, u in enumerate(task_uuids)], return_exceptions=True
        )
...
async def run_batch_pipeline(
    ...
    running_tasks_list = []
    add_batch_log(
        f"[系统] 流水线模式启动，并发数: {concurrency}，总任务: {len(task_uuids)}"
    )
    ...
    try:
        for i, task_uuid in enumerate(task_uuids):
            if task_manager.is_batch_cancelled(batch_id):
                with get_db() as db:
                    for remaining_uuid in task_uuids[i:]:
                        crud.update_registration_task(
                            db, remaining_uuid, status="cancelled"
                        )
            ...
            if i < len(task_uuids) - 1 and not task_manager.is_batch_cancelled(
                batch_id
            ):
...
async def create_mock_registration(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"无效的邮箱服务类型: {request.email_service_type}"
        )
...
async def start_registration(
    request: RegistrationTaskCreate, background_tasks: BackgroundTasks
):
    """
    启动注册任务
    ...
    with get_db() as db:
        task = crud.create_registration_task(
            db, task_uuid=task_uuid, proxy=request.proxy
        )
...
async def start_batch_registration(
    request: BatchRegistrationRequest, background_tasks: BackgroundTasks
):
    """
    启动批量注册任务
...
async def list_tasks(
    ...
    with get_db() as db:
        ...
        return TaskListResponse(total=total, tasks=[task_to_response(t) for t in tasks])
...
async def get_registration_stats():
    ...
    with get_db() as db:
        ...
        # 按状态统计
        status_stats = (
            db.query(RegistrationTask.status, func.count(RegistrationTask.id))
        ...
        today_count = (
            db.query(func.count(RegistrationTask.id))
            .filter(func.date(RegistrationTask.created_at) == today)
            .scalar()
        )
...
async def get_available_email_services():
    ...
    result = {
        ...
        "imap_mail": {"available": False, "count": 0, "services": []},
        "cloud_mail": {"available": False, "count": 0, "services": []},
    }
        ...
        for service in outlook_services:
            ...
            accounts.append(
                OutlookAccountForRegistration(
                    ...
                    registered_account_id=existing_account.id
                    if existing_account
                    else None,
                )
            )
...
async def start_outlook_batch_registration(
    request: OutlookBatchRegistrationRequest, background_tasks: BackgroundTasks
):
    """
    启动 Outlook 批量注册任务
