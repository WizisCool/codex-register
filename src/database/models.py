...
class Account(Base):
    ...
    token_sync_status = Column(
        String(20), default="not_ready"
    )  # 'not_ready', 'pending', 'synced'
    ...
    source = Column(
        String(20), default="register"
    )  # 'register' 或 'login'，区分账号来源
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cpa_uploaded_at": self.cpa_uploaded_at.isoformat()
            if self.cpa_uploaded_at
            else None,
            "newapi_uploaded": self.newapi_uploaded,
            "newapi_uploaded_at": self.newapi_uploaded_at.isoformat()
            if self.newapi_uploaded_at
            else None,
            "source": self.source,
            "subscription_type": self.subscription_type,
            "subscription_at": self.subscription_at.isoformat()
            if self.subscription_at
            else None,
            "created_at": self.created_at.isoformat() if self.created_at else None,
            "updated_at": self.updated_at.isoformat() if self.updated_at else None,
            "token_sync_status": self.token_sync_status,
            "token_sync_updated_at": self.token_sync_updated_at.isoformat()
            if self.token_sync_updated_at
            else None,
        }
...
class RegistrationTask(Base):
    ...
    email_service_id = Column(
        Integer, ForeignKey("email_services.id"), index=True
    )  # 使用的邮箱服务
...
class Codex2ApiService(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    name = Column(String(100), nullable=False)  # 服务名称
    api_url = Column(String(500), nullable=False)  # API URL (如 http://host)
    api_key = Column(Text, nullable=False)  # x-admin-key
    enabled = Column(Boolean, default=True)
    include_proxy_url = Column(Boolean, default=True)  # 是否上传 proxy_url 到 Codex2API
    priority = Column(Integer, default=0)  # 优先级
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
